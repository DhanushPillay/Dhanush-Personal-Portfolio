...
import "./toaster-button.css";

interface ToasterButtonProps {
  onClick?: () => void;
  type?: "button" | "submit" | "reset";
  status?: "idle" | "submitting" | "success" | "error";
}

export const ToasterButton: React.FC<ToasterButtonProps> = ({
  onClick,
  type = "button",
  status = "idle",
}) => {
  ...
  return (
    <button
      className={`toaster-tooltip-container ${status === "success" ? "is-active" : ""}`}
      onClick={onClick}
      type={type}
      aria-label="Send Message"
    >
      <span className="toaster-tooltip">{getTooltipText()}</span>
      <span className="toasterGroup">
        <svg
          className="toaster-svg"
          width="100%"
          height="100%"
          viewBox="0 0 88 50"
          fill="none"
